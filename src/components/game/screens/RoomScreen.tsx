...
type RoomMode = 'join' | 'create' | 'supervisor';
...
interface RoomScreenProps {
  ...
  onJoinSupervisorRoom?: (
    data: Pick<RoomAccessForm, 'roomCode' | 'password'>,
  ) => void | Promise<void>;
}
...
export default function RoomScreen({
  ...
  const isSupervisorAccount = accountRole === 'supervisor';
  ...
  const fetchRooms = async () => {
    ...
    try {
      const query =
        isSupervisorAccount && userEmail
          ? `?supervisorEmail=${encodeURIComponent(userEmail)}`
          : '';
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'join' || mode === 'supervisor') {
      if (!canJoin) return;

      const joinHandler =
        mode === 'supervisor' && onJoinSupervisorRoom
          ? onJoinSupervisorRoom
          : onJoinRoom;
      ...
      <div className="relative z-10 w-full max-w-3xl">
        ...
        >
          <div className="border-b border-cyan-400/10 bg-slate-900/80 px-6 py-5 md:px-8">
            ...
            <div className={`grid gap-3 ${isSupervisorAccount ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
              <ModeButton
                active={mode === 'join'}
              ...
              {isSupervisorAccount && (
                <ModeButton
                  active={mode === 'supervisor'}
                  title="Rejoin as Supervisor"
                  description="Return to a room you created"
                ...
                <SideInfoCard
                  ...
                  items={
                    mode === 'create'
                      ...
                      : mode === 'supervisor'
                        ? [
                            'Select one of your created rooms.',
                            'Enter the room password to issue fresh access.',
                            'Return directly to the Supervisor Dashboard.',
                          ]
                        : [
                    ...
                    {rooms.length === 0 ? (
                      ...
                      rooms.map((room) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            setMode(isSupervisorAccount ? 'supervisor' : 'join');
